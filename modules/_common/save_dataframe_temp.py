...
def save_dataframe_temp(lab_key: str, json_str: str) -> dict:
    """
    Guarda:
      modules/<lab_key>/dataframe/temp/<YYYY-MM-DD_HH-MM-SS>/
        - datos_validos.csv
    ...
    target_dir = os.path.join(base_modules, lab_key, "dataframe", "temp")
...
def _main_cli():
    if len(sys.argv) < 2:
        print(json.dumps({"ok": False, "error": "Falta lab_key"}))
        sys.exit(1)
    ...
    try:
        ...
        res = save_dataframe_temp(lab_key, json_str)
