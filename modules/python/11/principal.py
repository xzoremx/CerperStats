...
if all_values.size >= 2:
    ...
    # Pooled within-group std (repeatability)
    denom = 0
    num = 0.0
    group_means = []
    group_ns = []
    for n, m, s in col_stats:
        ...
        if n > 0 and np.isfinite(m):
            ...
            group_ns.append(int(n))

    s_pooled = None
    if denom > 0 and num >= 0:
        s_pooled = float(np.sqrt(num / denom))
        rsd_r = _safe_rsd_pct(media_global, s_pooled)

    # Between-group component + reproducibility
    if s_pooled is not None and len(group_means) >= 2:
        k = len(group_means)
        mean_overall = float(media_global)
        s_between_num = 0.0
        for n_i, m_i in zip(group_ns, group_means):
            s_between_num += n_i * ((m_i - mean_overall) ** 2)
        s_between = float(np.sqrt(s_between_num / max(k - 1, 1)))
        s_R = float(np.sqrt((s_pooled ** 2) + (s_between ** 2)))
        rsd_R = _safe_rsd_pct(media_global, s_R)
